#ifndef VAPIDJSON_PRETTYWRITER_H_
#define VAPIDJSON_PRETTYWRITER_H_
...
public:
	...
	*/
	PrettyWriter& SetIndent(Ch indentChar, unsigned indentCharCount) {
		VAPIDJSON_ASSERT(indentChar == ' ' || indentChar == '\t' || indentChar == '\n' || indentChar == '\r');
	...
	PrettyWriter& EndObject(SizeType memberCount = 0) {
		VAPIDJSON_ASSERT(Base::level_stack_.GetSize() >= sizeof(typename Base::Level));
		VAPIDJSON_ASSERT(!Base::level_stack_.template Top<typename Base::Level>()->inArray);
	...
	void PrettyPrefix(Type type) {
		if (Base::level_stack_.GetSize() != 0) { // this value is not at root
			...
			if (!level->inArray && level->valueCount % 2 == 0)
				VAPIDJSON_ASSERT(type == kStringType);  // if it's in object, then even number should be a name
			level->valueCount++;
		}
		else
			VAPIDJSON_ASSERT(type == kObjectType || type == kArrayType);
	}
